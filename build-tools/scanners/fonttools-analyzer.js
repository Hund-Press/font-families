...
const PYTHON_ENV_PATH = path.join(process.cwd(), 'font-tools-env', 'bin', 'python');
...
export async function analyzeFontFile(fontPath) {
    return new Promise((resolve, reject) => {
        const pythonProcess = spawn(PYTHON_ENV_PATH, [FONT_INSPECTOR_PATH, fontPath], {
...
export async function normalizeFont(fontToolsData, fontName, fontFile) {
    ...
    result.performance = {
        ...
        glyphDensity: basic.numGlyphs ? Math.round(fontToolsData.file.size / basic.numGlyphs) : null,
...
export async function extractCanonicalFamilyName(fontPath) {
    try {
        ...
        console.error(`Failed to extract family name from ${fontPath}:`, error.message);
        return null;
    }
}
...
export async function generateEnhancedFamilyMetadata(scannedData) {
    ...
    const firstFontPath = Object.values(scannedData.static)[0]?.path || 
    ...
    const canonicalName = await extractCanonicalFamilyName(path.resolve(firstFontPath));
    if (!canonicalName) {
    ...
    if (fontPaths.length > 0) {
        try {
            ...
            for (const range of analysis.features.unicodeRanges) {
                if (range.coverage > 0.3) { // 30% coverage threshold
