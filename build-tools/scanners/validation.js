...
export async function validateLicensing(familyData, distributionType = 'open') {
    console.log(`[validation] Checking ${familyData.name} for ${distributionType} distribution`);
...
async function validateOpenLicense(familyData) {
    ...
    if (!license) {
        console.warn(`[validation] ⚠️  ${familyData.name}: No license information found`);
        return false;
    }
    ...
    if (!isApprovedLicense) {
        console.warn(`[validation] ⚠️  ${familyData.name}: License '${license}' not approved for open distribution`);
        return false;
    }
    ...
    console.log(`[validation] ✅ ${familyData.name}: Valid open license (${license})`);
    return true;
}
...
async function validateLicenseFileExists(familyData) {
    ...
    const firstFont = Object.values(familyData.static || {})[0] || 
    ...
    for (const dir of searchDirs) {
        for (const licenseFile of licenseFiles) {
            try {
                ...
                console.log(`[validation] Found license file: ${path.join(dir, licenseFile)}`);
                return true;
            } catch (error) {
    ...
    for (const fontData of allFonts) {
        try {
            ...
            if (stats.size > 10 * 1024 * 1024) { // > 10MB
                results.warnings.push(`Large font file: ${fontData.path} (${Math.round(stats.size / 1024 / 1024)}MB)`);
            }
            
            if (stats.size < 1024) { // < 1KB
                results.errors.push(`Suspiciously small font file: ${fontData.path} (${stats.size} bytes)`);
                results.valid = false;
            }
            ...
            results.errors.push(`Cannot access font file: ${fontData.path} (${error.message})`);
            results.valid = false;
        }
    }
    
    if (results.valid) {
        console.log(`[validation] ✅ ${familyData.name}: File integrity validated (${results.checkedFiles} files)`);
    } else {
        console.error(`[validation] ❌ ${familyData.name}: File integrity issues found`);
        results.errors.forEach(error => console.error(`[validation]   - ${error}`));
    }
    
    if (results.warnings.length > 0) {
        results.warnings.forEach(warning => console.warn(`[validation]   ⚠️  ${warning}`));
    }
...
export async function validateUFRCompliance(familyPath, familyData) {
    ...
    if (hasPackage) {
        ...
        results.recommendations.push('Add package.json with version, author, and license fields');
    }
    ...
    if (organizedDirs >= 2) {
        results.score += 20;
        console.log(`[validation] ✅ UFR: Organized font directories (${organizedDirs}/${expectedDirs.length})`);
    } else {
        results.recommendations.push('Organize fonts into subdirectories (webfonts/, ttf/, otf/, variable/)');
    }
    ...
    try {
        ...
        results.recommendations.push('Add sources/ directory with font source files if available');
    }
    
    // Calculate compliance percentage
    const compliancePercentage = Math.round((results.score / results.maxScore) * 100);
    
    console.log(`[validation] UFR compliance for ${familyData.name}: ${compliancePercentage}% (${results.score}/${results.maxScore})`);
    ...
    if (results.recommendations.length > 0) {
        ...
        results.recommendations.forEach(rec => console.log(`[validation]   - ${rec}`));
    }
...
export function validateMetadata(familyData) {
    ...
    const completenessPercentage = Math.round((results.score / results.maxScore) * 100);
    results.completenessPercentage = completenessPercentage;
    
    if (results.complete) {
        console.log(`[validation] ✅ ${familyData.name}: Metadata complete (${completenessPercentage}%)`);
    } else {
        console.warn(`[validation] ⚠️  ${familyData.name}: Incomplete metadata (${completenessPercentage}%)`);
        results.missing.forEach(field => console.warn(`[validation]   Missing: ${field}`));
    }
 ...
 */
export async function validateFontFamily(familyPath, familyData, distributionType = 'open') {
    console.log(`[validation] Running comprehensive validation for ${familyData.name}`);
    ...
    results.overall = results.licensing && results.fileIntegrity.valid && results.metadata.complete;
