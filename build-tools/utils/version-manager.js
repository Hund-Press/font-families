...
export async function getCurrentVersion(explicitVersion = null, options = {}) {
    ...
    if (preferGitTag) {
        try {
            ...
            console.log('[version] No exact git tag found for HEAD, trying package.json');
        }
    }
    ...
    try {
        ...
        const version = includeVPrefix && !packageJson.version.startsWith('v') 
    ...
    // 4. Fallback
    const version = includeVPrefix && !fallbackVersion.startsWith('v') 
        ? `v${fallbackVersion}` 
        : fallbackVersion;
...
export async function isVersionSynced() {
    try {
        ...
        if (!synced) {
            console.warn(`[version] Version mismatch: package.json=${packageVersion}, git tag=${gitTag}`);
        }
