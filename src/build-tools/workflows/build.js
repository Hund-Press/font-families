...
import {
  generateCatalog,
  generateSubsetAPIs,
} from '../generators/catalog-generator.js'
...
async function build() {
  ...
  try {
    ...
    console.log(
      `[build] Restricted fonts: ${Object.keys(restrictedFonts).length}`
    )
...
async function scanAllFonts() {
  ...
  try {
    ...
    console.log(
      `[build] No open fonts directory found at ${BUILD_CONFIG.openFontsDir}`
    )
  }
  ...
  try {
    ...
    console.log(
      `[build] Scanning restricted fonts at ${BUILD_CONFIG.restrictedFontsDir}`
    )
    ...
    console.log(
      `[build] No restricted fonts directory found at ${BUILD_CONFIG.restrictedFontsDir}`
    )
  }
...
async function validateAllLicensing(openFonts, restrictedFonts) {
  ...
  for (const [familyKey, familyData] of Object.entries(openFonts)) {
    ...
    if (!isValidOpenLicense) {
      console.warn(
        `[build] ⚠️  Font family ${familyData.name} may not have valid open license`
      )
    }
  }
  ...
  for (const [familyKey, familyData] of Object.entries(restrictedFonts)) {
    console.log(
      `[build] Restricted font: ${familyData.name} (license: ${familyData.license || 'unknown'})`
    )
  }
}

/**
 * Generate root API index file
 * @param {string} apiDir - API directory path
 * @param {string} version - API version
 */
async function generateRootAPIIndex(apiDir, version) {
  const rootAPIIndex = {
  ...
  const rootIndexPath = path.join(apiDir, 'index.json')
  await fs.writeFile(rootIndexPath, JSON.stringify(rootAPIIndex, null, 2))
  console.log(`[api] Generated root API index: ${rootIndexPath}`)
}
...
async function generateAllCatalogs(openFonts, allFonts) {
  ...
  // Generate root API index
  await generateRootAPIIndex(BUILD_CONFIG.apiDir, version)
  ...
  try {
    ...
    for (const [familyKey, familyData] of Object.entries(openFonts)) {
      try {
        ...
        console.warn(
          `[build] ⚠️ Subset generation failed for ${familyData.key}: ${error.message}`
        )
      }
    }
...
async function generateAllModules(openFonts) {
  ...
  await generateModules(openFonts, BUILD_CONFIG.modulesDir, {
    ...
    cdnBaseUrl:
      'https://cdn.jsdelivr.net/gh/hund-press/font-families@{version}',
...
async function buildCatalogSite(openFonts) {
  ...
  console.log(
    `[build] Created Eleventy data file with ${Object.keys(openFonts).length} font families`
  )
...
async function validateSubsets() {
  ...
  try {
    ...
    for (const family of families) {
      ...
      try {
        const familyMetadataPath = path.join(
          subsetsDir,
          family,
          'metadata.json'
        )
          ...
          if (subsetConfig.status === 'generated') {
            ...
            try {
              ...
              if (filesValid) {
                validSubsets++
                console.log(
                  `[build] ✅ Subset ${family}/${subsetName} (${subsetMetadata.files?.length || 0} files)`
                )
              }
            } catch (error) {
              console.warn(
                `[build] ⚠️  Invalid subset: ${family}/${subsetName} - ${error.message}`
              )
            }
          }
        }
        ...
        console.warn(
          `[build] ⚠️  Could not validate subsets for ${family}: ${error.message}`
        )
      }
    }
    ...
    if (totalSubsets > 0) {
      console.log(
        `[build] ✅ Subset validation: ${validSubsets}/${totalSubsets} subsets valid`
      )
    } else {
