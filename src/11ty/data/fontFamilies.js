...
function loadFamiliesData() {
  try {
    ...
    for (const [familyKey, familyInfo] of Object.entries(
      indexData.families || {}
    )) {
      try {
        ...
        totalFiles +=
          (familyData.files?.static?.length || 0) +
          (familyData.files?.variable?.length || 0)
      } catch (error) {
        console.warn(
          `Could not load family data for ${familyKey}:`,
          error.message
        )
      }
    }
    ...
    console.warn(
      'Could not load families index, returning empty data:',
      error.message
    )
