...
import { SubsetGenerator, CONFIG } from '../build-tools/generators/subset-generator.js';
...
test('SubsetGenerator - Basic functionality', async (t) => {
    ...
    await t.test('should have correct character set definition', () => {
        ...
        assert.strictEqual(charSet.characterCount, 100, 'Should have 100 characters');
        assert.ok(charSet.characters.includes('A'), 'Should include uppercase letters');
        assert.ok(charSet.characters.includes('a'), 'Should include lowercase letters');
        ...
        assert.ok(charSet.characters.length >= 90, 'Should have at least 90 characters');
        assert.ok(charSet.characters.length <= 110, 'Should have at most 110 characters');
    });
    ...
    await t.test('should load family metadata', async () => {
        ...
        assert.strictEqual(metadata.family, TEST_FAMILY, 'Should have correct family name');
        assert.ok(metadata.subsets[TEST_SUBSET], 'Should have min-chars subset config');
    });
...
test('Subset Files - Validation', async (t) => {
    ...
    await t.test('subset files should exist and have reasonable sizes', async () => {
        const expectedFiles = [
        ...
        for (const filename of expectedFiles) {
            ...
            try {
                ...
                assert.ok(stats.size < 50000, `${filename} should be smaller than 50KB (got ${stats.size} bytes)`);
                
                // Min size check - should be at least 2KB for basic font structure
                assert.ok(stats.size > 2000, `${filename} should be at least 2KB (got ${stats.size} bytes)`);
            } catch (error) {
...
test('Character Set Coverage', async (t) => {
    await t.test('min-chars should cover essential typography', () => {
        ...
        for (let i = 65; i <= 90; i++) { // A-Z
            assert.ok(chars.includes(String.fromCharCode(i)), `Should include ${String.fromCharCode(i)}`);
        }
...
test('Build Integration', async (t) => {
    await t.test('generated modules should include subset information', async () => {
        ...
        try {
            ...
            assert.ok(moduleContent.includes('_subsets'), 'Module should include subset CDN path');
        } catch (error) {
            assert.fail(`Module file should exist and contain subset info: ${error.message}`);
        }
    });
});